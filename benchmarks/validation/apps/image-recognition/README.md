# Image-Recognition Application (VAL-017)

A customer-style image-classification application: submitted images
classified against a fixed label vocabulary, through Zeck's public SDK
boundary.

## What it does

Submits pinned `image-recognition.classify.v1` corpus tasks (`kind:
"classify-image"` with a fixture key and label set), awaits async
completion, retrieves the result package and asserts the deterministic
outcome contract. The corrupted-image edge row asserts the honest
failure contract: terminal `FAILED`, verification `FAIL` — the provider
rejects genuinely undecodable media and the platform never fabricates a
label.
