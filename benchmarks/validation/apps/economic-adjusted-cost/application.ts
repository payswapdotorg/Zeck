/**
 * The economic-adjusted-cost customer application (VAL-044).
 *
 * A real customer-style application: one pinned adjusted-cost
 * synthesis row per run — the row's FAMILY (quality-adjusted |
 * latency-adjusted | failure-adjusted), the PRE-REGISTERED ARM SET
 * (digest references to the three recorded control arms' corpora),
 * the statistical minimums, the Wilson configuration and the expected
 * comparability verdict REFERENCE — submitted through Zeck's public
 * SDK boundary. The application exercises the customer side of the
 * adjusted-cost contract:
 *
 ...
 *     the honest verdict rows — comparable, honestly-incomparable or
 *     refused-below-minimum alike, since the honest verdict IS the
 *     verified outcome; FAILED when any mechanical criterion failed —
 *     including the six adversarial probe rows whose denatured input
 ...
 *     customer boundary);
 *   * the task body carries REFERENCES ONLY (the family, the arm-set
 *     digest references, the minimums, the Wilson level) — never an
 *     inline list price, never a recorded outcome copy (the platform
 *     resolves the corpora and the manifests through their
 *     registries).
 *
...
import { ADJUSTED_CORPUS, submissionKey, taskBodyFor } from "./corpus";

/** The app's full run outcome (evidence + assertions + outcome contract). */
export interface AdjustedAppResult {
  readonly evidence: HarnessEvidence;
  readonly passed: boolean;
  readonly adjustedCriteria: readonly LabVerificationCriterion[];
...
/** The app's pinned corpus slice (`economic-adjusted-cost.experiment.v1` rows). */
export const ADJUSTED_TASKS = ADJUSTED_CORPUS.map((row) => ({
  kind: "economic-adjusted-cost.experiment.v1",
  rowId: row.rowId,
...
/**
 * Run the economic-adjusted-cost application end to end over one
 ...
 * pricing and never copies recorded outcomes (the row declares the
 * digest references; the platform resolves the corpora).
 */
export async function runAdjustedApp(options: {
...
}): Promise<AdjustedAppResult> {
  const row = ADJUSTED_CORPUS[options.taskIndex];
  if (row === undefined) {
    throw new Error("the pinned adjusted-cost synthesis task slice is empty");
  }
  ...
  //    customer boundary) + the expected terminal met. The synthesis
  //    row satisfies the economic row contract's shape (the VAL-040
  //    grammar rides the arm entries the references resolve to).
  const adjustedCriteria = verifyEconomicAppContract({
    row: row as unknown as EconomicCorpusRow,
    terminal: observedTerminal,
    verificationStatuses,
  });
  const contractPassed = adjustedCriteria.every((criterion) => criterion.status === "PASS");
