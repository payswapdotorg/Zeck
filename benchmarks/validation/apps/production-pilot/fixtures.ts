 ...
 *
 * The controlled world the pilot driver and the app execute against:
 *
 ...
 *     pilot observation package at the result boundary). The
 *     discrimination knobs: `probe` forces an adversarial pilot
 *     shape onto a declared row (the controlled fakes of the
 *     discrimination battery — a cherry-picked window, a dropped
 *     shift, a double-driven resume, a normalized drift, a hidden
 *     incident, a hidden residual, a boundary leak);
 ...
 *     gate is declared (the live window demands the
 *     operator-authorized REAL rail; a fake serving it would
 *     fabricate a live pilot window);
...
import type { PilotProbeKind, ProductionPilotCorpusRow } from "./corpus";
import { PRODUCTION_PILOT_CORPUS } from "./corpus";
import { pilotObservationFor } from "./driver";
 ...
 * semantics at the customer boundary (mirroring the VAL-049/VAL-050
 ...
 * carrying the pilot observation).
 */
export function createPilotFakeApiWorld(options: {
  readonly clock: TickClock;
  /**
   * Force an adversarial pilot probe onto the row (the
  ...
  const rowOf = (rowId: string): ProductionPilotCorpusRow | null =>
    PRODUCTION_PILOT_CORPUS.find((candidate) => candidate.rowId === rowId) ?? null;
  ...
  const transport: TransportImplementation = async (input: unknown, init?: unknown) => {
    ...
    if (url.endsWith("/executions") && method === "POST") {
      ...
      // THE LIVE RAIL HONESTY: the offline fake world never serves a
      // live-gated row — the live pilot window demands the
      // operator-authorized REAL rail (a fake serving it would
      // fabricate a live window).
      if (row.liveGate !== undefined) {
        return jsonResponse(503, {
          code: "CAPABILITY_UNAVAILABLE",
          message:
            "the offline fake world never serves the live rail: the live pilot window demands the operator-authorized REAL platform path",
          retryable: false,
        });
      }
    ...
    if (execMatch !== null && method === "GET") {
      ...
      return jsonResponse(200, {
        ...
        environmentId: null,
        status: row.status,
        task: { kind: "production-pilot.observation.v1", input: "pilot" },
        constraints: null,
        metadata: {},
    ...
    if (resultMatch !== null && method === "GET") {
      ...
      // The pilot observation package: the honest trace over the
