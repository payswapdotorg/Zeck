/**
 * The economic-substrate-runtime customer application (VAL-046).
 *
 * A real customer-style application: one pinned substrate-cost
 * synthesis row per run — the row's FAMILY (substrate-cost-per-run |
 ...
 * substrate telemetry), the composed ARM set (digest references into
 * the recorded control arms' corpora), the statistical minimums, the
 * Wilson configuration and the expected comparability verdict
 * REFERENCE — submitted through Zeck's public SDK boundary. The
 * application exercises the customer side of the substrate-cost
 * contract:
 *
 ...
 *     including the seven adversarial probe rows whose denatured
 *     claim shapes FAIL their named criteria);
 ...
 *     customer boundary);
 *   * the task body carries REFERENCES ONLY (the family, the window-set
 *     digest references, the arm-set digest references, the minimums,
 *     the Wilson level) — never an inline substrate price, never a
 *     recorded telemetry copy (the platform resolves the corpora and
 *     the manifests through their registries).
 *
...
import { SUBSTRATE_CORPUS, submissionKey, taskBodyFor } from "./corpus";

/** The app's full run outcome (evidence + assertions + outcome contract). */
export interface SubstrateAppResult {
  readonly evidence: HarnessEvidence;
  readonly passed: boolean;
  readonly substrateCriteria: readonly LabVerificationCriterion[];
...
/** The app's pinned corpus slice (`economic-substrate-runtime.experiment.v1` rows). */
export const SUBSTRATE_TASKS = SUBSTRATE_CORPUS.map((row) => ({
  kind: "economic-substrate-runtime.experiment.v1",
  rowId: row.rowId,
  ...
  windowSetSize: row.windowSet.length,
...
/**
 * Run the economic-substrate-runtime application end to end over one
 ...
 * pricing and never copies recorded telemetry (the row declares the
 * digest references; the platform resolves the corpora).
 */
export async function runSubstrateApp(options: {
...
}): Promise<SubstrateAppResult> {
  const row = SUBSTRATE_CORPUS[options.taskIndex];
  if (row === undefined) {
    throw new Error("the pinned substrate-cost synthesis task slice is empty");
  }
  ...
  const substrateCriteria = verifyEconomicAppContract({
    row: row as unknown as EconomicCorpusRow,
    terminal: observedTerminal,
    verificationStatuses,
  });
  const contractPassed = substrateCriteria.every((criterion) => criterion.status === "PASS");
