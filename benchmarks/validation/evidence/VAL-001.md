...
Validation evidence documents live under `benchmarks/validation/evidence/`
(inside the validation laboratory's declared surfaces). The product
program's e11 containment proofs diff the whole `spec/` tree against the
branch merge-base, so validation PRs keep every added file out of `spec/`
(the Architect lands validation spec/state files on main directly; the
validation program adds nothing under `spec/` itself).
