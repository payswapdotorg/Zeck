...
| `ZECK_PG_TEST_URL=… bun run test` (FULL suite) | **1760/1760 (166 files) in EVERY run, zero failing tests, zero unhandled errors** — twice consecutively at the implementation head `3cb8351` AND twice consecutively (exit 0, exit 0) at the exact final evidence head `01c4151`. Run 1 at `01c4151` fired the DOCUMENTED teardown transient (a `pg_terminate_backend` permission error during disposable-database DROP — the 57P01 teardown-transient class of WORK-010/013/014/015/016/017: ALL 1760 tests passing, the suite exits 1 only because cleanup failed); runs 2+3 were consecutive clean exit-0 (`/tmp/full-023-final2.log`, `/tmp/full-023-final3.log`). Main at pickup was 1683/1683 (161 files) green; this branch's delta is +77 tests / +5 files (exactly WORK-023's tests) |
