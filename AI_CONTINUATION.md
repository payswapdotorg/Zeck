...
- ALL issued work-order specs (VAL-001..004) are COMPLETE. Frontier: eligible=[], inFlight=[] — VAL-005/VAL-008/VAL-009 are dependency-complete and first in line, but every remaining work order AWAIT ARCHITECT-ISSUED SPECS under spec/validation-work-orders/. The program continues only when the Architect issues the next specs (dispatch discipline: never implement an unspec'd work order).
