...
describe("the real repository manifest set (WORK-042 D-01)", () => {
  ...
  test("maps every concern to a provider with an owning port and substitution target", () => {
    ...
    // D-03 (cloudflare-queues) landed with WORK-044 and D-04
    // (cloudflare-workflows) landed with WORK-045: both established
    // with real port contracts; every declared port is landed.
    expect(planned.map((p) => p.plannedPhase).sort()).toEqual([]);
  ...
  test("scopes secret references per environment with the local set", () => {
    ...
    expect(manifest.secretReferences.local.map((r) => r.name).sort()).toEqual([
      ...
      // PPR-009 appended the livekit keypair references; PPR-010 appended
      // the socketio auth-secret reference (additive manifests).
      "experience-token",
      "livekit-api-key",
      "livekit-api-secret",
      "socketio-auth-secret",
      "transport-token",
    ]);
    for (const environment of ["preview", "staging", "production"] as const) {
      expect(manifest.secretReferences[environment]).toHaveLength(15);
    }
