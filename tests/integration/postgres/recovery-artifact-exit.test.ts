...
import { S3ObjectStoreError, createS3ObjectStore } from "../../../src/platform/object-store/s3-object-store";
import {
...
definePgSuite(
  "the artifact-recovery / provider-exit drill (WORK-048 D-07 AC2+AC5)",
  (ctx) => {
    ...
    beforeAll(async () => {
      ...
      if (digestOf(artifactBytes) !== adoption.rows[0]?.artifact_digest) {
        throw new Error("the artifact bytes do not hash to the authoritative digest (fixture defect)");
      }
