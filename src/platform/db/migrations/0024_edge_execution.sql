...
CREATE OR REPLACE FUNCTION edge.ec_commands_lifecycle_guard() RETURNS trigger AS $$ BEGIN IF OLD.status IN ('denied','settled','failed','invalidated','conflicted') AND NEW IS DISTINCT FROM OLD THEN RAISE EXCEPTION 'edge command % is terminal-immutable in status %', OLD.id, OLD.status; END IF; IF OLD.status = 'authorized' AND NEW.status NOT IN ('authorized','dispatched','failed','invalidated') THEN RAISE EXCEPTION 'edge command % cannot move from authorized to %', OLD.id, NEW.status; END IF; IF OLD.status = 'dispatched' AND NEW.status NOT IN ('dispatched','settled','failed','conflicted') THEN RAISE EXCEPTION 'edge command % cannot move from dispatched to %', OLD.id, NEW.status; END IF; IF OLD.status = 'authorized' AND NEW.status = 'authorized' AND NEW IS DISTINCT FROM OLD THEN RAISE EXCEPTION 'an authorized edge command has no mutable fields (command %)', OLD.id; END IF; IF OLD.dispatched_at IS NOT NULL AND NEW.dispatched_at IS DISTINCT FROM OLD.dispatched_at THEN RAISE EXCEPTION 'edge command % dispatch timestamp is write-once', OLD.id; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql;
