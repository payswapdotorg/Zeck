/**
 * Connections realtime secret-mediation adapter (deployments module;
 * WORK-024).
 *
 * Implements the deployments module's REQUIRED `RealtimeSecretMediation`
 * port against the REAL connections module catalog (the WORK-003
 * authority that owns BYOK credential references). Mediation here means
 * ACCESS ARBITRATION + REFERENCE-ONLY outcomes: the realtime session
 * service learns ONLY whether the rail channel's credential reference
 * is resolvable and active, and receives an OPAQUE grant reference —
 * raw secret values NEVER cross into the deployments module. A real
 * rail adapter materializes plaintext inside its own scope through the
 * connections vault (the models-gateway "secrets last" discipline);
 * that materialization is the ADAPTER's concern and is explicitly
 * UNVERIFIED in this environment (no provider credentials exist — see
 * docs/work-items/WORK-024.md).
 *
 ...
 * credential fails closed before any rail delivery.
 *
...
export function createConnectionsRealtimeSecretMediation(
  catalog: ConnectionCatalog,
): RealtimeSecretMediation {
  return {
    async mediate(
      request: RealtimeSecretMediationRequest,
    ): Promise<RealtimeSecretMediationOutcome> {
