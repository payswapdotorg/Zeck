...
Implement provider-neutral connections, BYOK and federated provider adapters.
...
- implementation evidence in `docs/work-items/WORK-003.md`
...
1. Define provider-neutral Connection and ModelProvider ports with no provider SDK types in public contracts.
2. Implement OpenRouter as one adapter/rail and at least one direct provider adapter behind the same contract.
3. Persist BYOK as a secret reference; never return or log plaintext credentials.
4. Normalize provider usage, streaming, structured output and provider-error categories.
5. Prove provider failure and quality/verification failure are distinct durable outcomes.
