...
Implement policy-gated, evidence-producing tool execution.
...
Requires: WORK-006, WORK-007, WORK-008
...
- implementation evidence in `docs/work-items/WORK-010.md`
...
1. Define a provider-neutral Tool contract with input/output schema and declared capabilities/side effects.
2. Implement invocation authorization through the policy gate before adapter dispatch.
3. Persist tool request, outcome, timing, artifact references and error class as execution evidence.
4. Prevent tools from directly mutating customer-domain workflow or platform authority state.
5. Add a negative test proving a denied tool is never invoked.
