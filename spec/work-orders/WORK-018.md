# WORK-018 — Tool synthesis and validated ephemeral programs
...
Add validated program synthesis inside the governed tool abstraction.
...
Requires: WORK-010, WORK-012, WORK-014, WORK-017
...
- implementation evidence in `docs/work-items/WORK-018.md`
...
1. Represent synthesized tools as ephemeral, content-addressed artifacts with explicit schemas and capabilities.
2. Compile/run synthesized programs only inside the sandbox manager.
3. Require static validation + runtime tests before a synthesized program can be used as a tool.
4. Persist source, build digest, test evidence and execution provenance.
5. Prevent synthesized code from gaining capabilities not granted by policy.
