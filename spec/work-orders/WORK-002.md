...
Establish authoritative actor/application/environment scope and server-side tenant isolation.
...
- implementation evidence in `docs/work-items/WORK-002.md`
...
1. Define actor, application, environment and membership authorization contracts.
2. Persist tenant/application ownership with database constraints that prevent cross-tenant ownership ambiguity.
3. Resolve application scope server-side on every protected command; callers may not select arbitrary tenant scope.
4. Prove a cross-tenant read/write is rejected before downstream module execution.
5. Provide idempotent identity/membership mutation semantics.
